 ...
 *
 * C:\xampp\php\PEAR\HTML\Template\IT.php
 *
 * PHP version 4
 *
...
class HTML_Template_IT
{
    ...
    {	
        ...
        return $PEAR->raiseError(
			$this->errorMessage(IT_UNKNOWN_OPTION) . ": '{$option}'<br>in File: " . __FILE__ . "<br>Line: " . __LINE__, 
			IT_UNKNOWN_OPTION
		);
    }
    ...
    function get($block = '__global__')
    {
        ...
        if (!isset($this->blocklist[$block])) {
            $this->err[] = $PEAR->raiseError(
				$this->errorMessage(IT_BLOCK_NOT_FOUND, $block, __FILE__, __LINE__),
				IT_BLOCK_NOT_FOUND
			);
            return '';
        }
        ...
        if (preg_match_all($this->blockRegExp, $string, $regs, PREG_SET_ORDER)) {
            foreach ($regs as $k => $match) {
                ...
                if (isset($this->blocklist[$blockname])) {
					...
					$this->err[] = $PEAR->raiseError(
						$this->errorMessage(IT_BLOCK_DUPLICATE, $blockname, __FILE__, __LINE__),
						IT_BLOCK_DUPLICATE
					);
#					return "";
	...
	$filename = $this->fileRoot . $filename;
        ...
        if (!($fh = @fopen($filename, 'r'))) {
            $this->err[] = $PEAR->raiseError(
				$this->errorMessage(IT_TPL_NOT_FOUND, $filename, __FILE__, __LINE__),
				IT_TPL_NOT_FOUND
			);
            return "";
        }
     ...
     * not recognized
     */
    function errorMessage($value, $blockname = '', $file = __FILE__, $line = __LINE__)
    {
		global $PEAR;
        ...
        if (!isset($errorMessages)) {
            $errorMessages = array(
                IT_OK                       => '',
                IT_ERROR                    => '<br>unknown error',
                IT_TPL_NOT_FOUND            => '<br>Cannot read the template file "' . $blockname . '"<br>in File: ' . $file . '<br>in Line: ' . $line,
                IT_BLOCK_NOT_FOUND          => '<br>Cannot find block "' . $blockname . '"<br>in File: ' . $file . '<br>in Line: ' . $line,
                IT_BLOCK_DUPLICATE          => '<br>The name of a block must be unique within a template.' .
                                               '<br>Found "' . $blockname . '" twice.'.
                                               '<br>Unpredictable results may appear.',
                IT_UNKNOWN_OPTION           => '<br>Unknown option'
            );
        }
